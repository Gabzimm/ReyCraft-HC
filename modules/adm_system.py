...
class ListaCargosView(ui.View):
    def __init__(self, cog, ctx, modo, current_page=0):
        ...
        for role in self.current_roles:
            ...
            self.dropdown.add_option(label=role.name[:100], value=role.name, emoji=emoji)
        ...
        if self.total_pages > 1:
            if current_page > 0:
                prev = ui.Button(label="◀️ Anterior", style=ButtonStyle.secondary, row=1)
                prev.callback = self.previous_page
                self.add_item(prev)
            if current_page < self.total_pages - 1:
                nxt = ui.Button(label="Próxima ▶️", style=ButtonStyle.secondary, row=1)
...
class AdmPainelView(ui.View):
    ...
    async def add_adm(self, interaction: discord.Interaction, button: ui.Button):
        ...
        digitar = ui.Button(label="✏️ Digitar Nome", style=ButtonStyle.primary)
        async def digitar_cb(interaction):
            modal = EscolherCargoModal(self.cog, self.ctx, modo="add")
            await interaction.response.send_modal(modal)
        digitar.callback = digitar_cb
        
        selecionar = ui.Button(label="📋 Selecionar da Lista", style=ButtonStyle.secondary)
        async def selecionar_cb(interaction):
    ...
    async def remove_adm(self, interaction: discord.Interaction, button: ui.Button):
        ...
        embed = discord.Embed(
            ...
            description=f"**📋 ADMs atuais:** {', '.join(adm_roles)}",
